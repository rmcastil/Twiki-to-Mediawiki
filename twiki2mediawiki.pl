...
my @ignoredVars = qw(ACTIVATEDPLUGINS ADDTOHEAD ALLOWLOGINNAME ALLVARIABLES AUTHENTICATED AUTHREALM BACKUPRESTORE BASETOPIC BASEWEB BUTTON CALCULATE CALENDAR CANLOGIN CHARSET CLEAR COLORPICKER COMMENT CONTENTMODE CRYPTTOKEN DATEPICKER DEFAULTURLHOST DISKID EDITFORMFIELD ENCODE ENDSECTION ENDTAB ENDTABPANE ENDTWISTY ENDTWISTYTOGGLE ENTITY ENV EXAMPLEVAR EXEC FAILEDPLUGINS FORMFIELD GET GMTIME GROUPS HEADLINES HIDE HIDEINPRINT HOMETOPIC HTTP HTTP_HOST HTTPS IF INCLUDINGTOPIC INCLUDINGWEB INTURLENCODE JAVASCRIPT_TEXT JQBUTTON JQCLEAR JQENDTAB JQENDTABPANE JQIMAGESURLPATH JQSCRIPT JQTAB JQTABPANE JQTHEME JQTOGGLE LABLOG LANG LANGUAGE LANGUAGES LOCALSITEPREFS LOGIN LOGINURL LOGOUT LOGOUTURL MAKETEXT MDREPO METASEARCH NOFOLLOW NOP NOTIFYTOPIC OTOPIC OWEB PARENTTOPIC PLUGINDESCRIPTIONS PLUGINVERSION QUERYPARAMS QUERYSTRING REDIRECT RELATIVETOPICPATH REMOTE_ADDR REMOTE_PORT REMOTE_USER RENDERHEAD REVARG REVINFO REVTITLE SCRIPTNAME SCRIPTSUFFIX SCRIPTURL SCRIPTURLPATH SEARCH SEP SERVERTIME SESSIONLOGON SESSION_VARIABLE SET SETGETDUMP SITESTATISTICSTOPIC SPACEOUT STARTSECTION STATISTICSTOPIC SYSTEMWEB TAB TABLE TABPANE TAGME TAGMEPLUGIN_USER_AGNOSTIC TGPOPUP TOC TOGGLE TOPICLIST TOPICMAP TOPICTITLE TRASHWEB TWIKIADMINLOGIN TWISTY TWISTYBUTTON TWISTYHIDE TWISTYSHOW TWISTYTOGGLE URLENCODE URLPARAM USERINFO USERMANAGER USERNAME USERPREFSTOPIC USERSWEB VAR VARIABLES WATCHLIST WEBLIST WEBPREFSTOPIC WIKINAME WIKIPREFSTOPIC WIKIUSERNAME WIKIUSERSTOPIC WIKIVERSION WIKIWEBMASTER WIKIWEBMASTERNAME WYSIWYG_SECRET_ID WYSIWYG_TEXT);
